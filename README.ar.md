...
`تـطبيق[نـص, بـعدم[T]]`، `تـطبيق[نـص, مصفوفة[بـعدم[T]]]`، و`مصفوفة[تـطبيق[نـص, بـعدم[T]]]`، حيث `T` هو
`نـص`، `ثـنائي`، `صحيح[32]`، `صحيح[64]`، `عـائم[32]`، أو `عـائم[64]`. القيمة غير المحددة من نمط `بـعدم`
تُحوَّل إلى `null`.
